...
func (s *Server) setupRoutes() {
	...
	openAIUsageLimit := middleware.UsageLimitMiddleware(s.usageLimitTracker, middleware.ProtocolOpenAI)
	s.engine.GET("/v1/realtime", realtimeAuth, openAIUsageLimit, s.codexLiveHandler.HandleRealtimeWebsocket)
	s.engine.POST("/v1/realtime", realtimeAuth, openAIUsageLimit, s.codexLiveHandler.Handle)
	s.engine.POST("/v1/realtime/calls", realtimeAuth, openAIUsageLimit, s.codexLiveHandler.Handle)
	s.engine.GET("/v1/realtime/calls/:call_id", realtimeAuth, openAIUsageLimit, s.codexLiveHandler.HandleSideband)
	s.engine.POST("/v1/realtime/client_secrets", standardAuth, openAIUsageLimit, s.codexLiveHandler.CreateClientSecret)
	s.engine.POST("/v1/realtime/sessions", standardAuth, openAIUsageLimit, s.codexLiveHandler.CreateLegacySession)
	s.engine.POST("/v1/realtime/transcription_sessions", standardAuth, openAIUsageLimit, s.codexLiveHandler.HandleTranscriptionSession)
	s.engine.GET("/v1/realtime/translations", realtimeAuth, openAIUsageLimit, s.codexLiveHandler.HandleTranslation)
	s.engine.POST("/v1/realtime/translations", realtimeAuth, openAIUsageLimit, s.codexLiveHandler.HandleTranslation)
	s.engine.POST("/v1/realtime/translations/client_secrets", standardAuth, openAIUsageLimit, s.codexLiveHandler.HandleTranslation)
	s.engine.POST("/v1/realtime/calls/:call_id/hangup", standardAuth, openAIUsageLimit, s.codexLiveHandler.HandleHangup)
	s.engine.POST("/v1/realtime/calls/:call_id/accept", standardAuth, openAIUsageLimit, s.codexLiveHandler.HandleSIPControl)
	s.engine.POST("/v1/realtime/calls/:call_id/reject", standardAuth, openAIUsageLimit, s.codexLiveHandler.HandleSIPControl)
	s.engine.POST("/v1/realtime/calls/:call_id/refer", standardAuth, openAIUsageLimit, s.codexLiveHandler.HandleSIPControl)
