...
func TestDirectRealtimeRoutesEnforceUsageLimitsAfterAuth(t *testing.T) {
	server := newTestServer(t)
	server.usageLimitTracker.SetLimits(map[string]usagelimit.Limits{
		"test-key": {MaxRequests: 1},
	})

	unauthorized := httptest.NewRequest(http.MethodPost, "/v1/realtime/calls", nil)
	unauthorizedRecorder := httptest.NewRecorder()
	server.engine.ServeHTTP(unauthorizedRecorder, unauthorized)
	if unauthorizedRecorder.Code != http.StatusUnauthorized {
	...
	first := httptest.NewRequest(http.MethodPost, "/v1/realtime/calls", nil)
	first.Header.Set("Authorization", "Bearer test-key")
	firstRecorder := httptest.NewRecorder()
	server.engine.ServeHTTP(firstRecorder, first)
	if firstRecorder.Code != http.StatusServiceUnavailable {
		t.Fatalf("first authorized status = %d, want %d; body=%s", firstRecorder.Code, http.StatusServiceUnavailable, firstRecorder.Body.String())
	}

	limited := httptest.NewRequest(http.MethodPost, "/v1/realtime/calls", nil)
	limited.Header.Set("Authorization", "Bearer test-key")
	limitedRecorder := httptest.NewRecorder()
	server.engine.ServeHTTP(limitedRecorder, limited)
	if limitedRecorder.Code != http.StatusTooManyRequests {
		t.Fatalf("limited status = %d, want %d; body=%s", limitedRecorder.Code, http.StatusTooManyRequests, limitedRecorder.Body.String())
	}
	if got := limitedRecorder.Header().Get("X-RateLimit-Remaining"); got != "0" {
...
func TestExampleAPIKeySafeModeShowsWarningAndKeepsManagement(t *testing.T) {
	...
	t.Run("management button query opens control panel", func(t *testing.T) {
		...
		if got := rr.Header().Get("X-CPAMC-Commit"); got != "187a1d69d1169843cc5a958ab813453fc6422439" {
